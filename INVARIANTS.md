...
the time actually streamed and tokens from usage if any chunk carried it
else an estimate from the streamed text — counted as neither a circuit
...
**Asserted by:** `tests/test_unpriced.py` (36 tests covering all three
...
(`token` alone) or **self-hosted** (`control_plane_url`, `token=""` for no
...
**The bound.** The mirror is generated by `scripts/export_sdk.py` from an
explicit allowlist (`runbound/**`, `tests/**`, `examples/**`, this file and
its siblings, `pyproject.toml`, and a small set of generated CI/packaging
files) — anything not on that list is excluded by construction, not by
care taken while writing a commit. The exporter also runs a lint over every
file it does export and fails the export outright if any of them still
...
retired internal name. A public mirror that fails that lint is never
published.

**Asserted by:** `tests/test_export_sdk.py`, which plants a file outside the
allowlist and a file containing each lint token and asserts both are kept
out of — or fail — the export.
